...
class Greedy(nj.Module):
  ...
  def __init__(self, wm, act_space, config):
    ...
    # overwriting the reward function
    rewfn = lambda s: self.ac.disc_reward(s)[1:]
    self.ac.critics['extr'] = agent.VFunction(rewfn, config, name='critic')
  ...
  def train(self, imagine, start, context, expert, observe):
    ...
    return self.ac.train(imagine, start, context, expert, observe)
...
class Explore(nj.Module):
  ...
  def __init__(self, wm, act_space, config):
    ...
    # overwriting the reward function
    rewfn = lambda s: self.ac.disc_reward(s)[1:]
    self.ac.critics['extr'] = agent.VFunction(rewfn, config, name=key)
